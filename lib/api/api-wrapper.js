...
class ApiWrapper {
  ...
  _joinHasManyRelations (parentRows, relations) {
    ...
    return parentRows.map((parentRow) => {
      relations.forEach((rel) => {
        parentRow[rel.modelFieldName] = filter(rel.rows, [rel.fkAs, parentRow.id])
        .map((relRow) => {
          relRow[rel.parentModelFieldName] = { id: parentRow.id } // reverse connection
          delete relRow[rel.fkAs]
          return relRow
        })
      })
      return parentRow
  ...
  _fetchHasManyRelations (parentWhere) {
    ...
    return Promise.all(this._hasManyRelations.map((rel) => {
      ...
      .then((castRows) => {
        return {
          modelFieldName: rel.modelFieldName,
          // parentModelFieldName, TODO when sqlBuilder.getModelFieldName() is implemented
          fkAs: rel.fkAs,
          rows: castRows
        }
      })
