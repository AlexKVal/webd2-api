...
class ApiWrapper {
  constructor (attrs, registryMockAsSecondArg) {
    ...
    this.belongsToDescriptors = this.model.sqlBuilder.belongsToDescriptors
    this.hasManyDescriptors = getHasManyDescriptors(this.model.schema)
    ...
    if (serializer) {
      ...
      this.serializer = new Serializer({
        ...
        attributesOfRelations: attributesOfRelations(this.registry, this.belongsToDescriptors)
      })
    }
  ...
  _fetchRelations (parentRows, parentWhere) {
    ...
    return Promise.all(this.belongsToDescriptors.map((rel) => {
  ...
  _fetchHasManyRelations (parentWhere) {
    ...
    return Promise.all(this.hasManyDescriptors.map((rel) => {
      const relationModel = this.registry.model(rel.relationModelName)
