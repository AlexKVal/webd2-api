 ...
 *
 * userApi.apiFetchMany({withRelated: true, where: {hide:false}, orderBy: 'name'})
...
class ApiWrapper {
  ...
  readMany (req, res, next) {
    ...
    options.withRelated = !!(req.query || {}).related

    this.apiFetchMany(options)
  ...
  connect (router, options) {
    ...
    if (opts.read) {
      forMany.get(this.readMany)
      forOne.get(this.readOne)
    }

    if (opts.create) forMany.post(this.create)
    if (opts.update) forOne.patch(this.update)
    if (opts.delete) forOne.delete(this.delete)
