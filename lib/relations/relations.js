...
class Relations {
  ...
  _fetchHasMany (parentWhere, options) {
    debug(`${this.modelName}:_fetchHasMany`)

    const { onlyIDs } = options || {}
    ...
    return Promise.all(this.hasManyDescriptors.map((relDescriptor) => {
      ...
      return relationModel.selectMany({
        fieldsOnly: onlyIDs ? 'idAndRelations' : undefined,
        whereIn: {
  ...
  justEmbedIds (parentRows, parentWhere) {
    debug(`${this.modelName}:justEmbedIds`)

    return this._fetchHasMany(parentWhere, {onlyIDs: true})
    .then((relationsData) => this._embedHasMany(parentRows, relationsData))
    .then((partialResult) => this.justTransformIDs(partialResult))
  }
}
