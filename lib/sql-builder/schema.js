...
function Schema (obj) {
  ...
  this.relations = {}
  this.belongsToKeys = [] // this.relations[key] -> 'belongsTo' relation
    ...
    if (typeof val === 'object') { // relations
      if (val.belongsTo) {
        ...
        if (!(relation instanceof Object) || (typeof relation.name) !== 'string') {
          throw new TypeError("belongsTo should be a model: registry['model-name']")
        }
        val.fkAs = val.fkAs || foreignKey(relation.name)
        this.attributes.push(val.fkAs)
        this.belongsToKeys.push(key)
      }
