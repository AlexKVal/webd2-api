...
class SqlBuilder {
  ...
  getModelFieldName (modelName) {
    const belongsToRelations = getBelongsToRelations(this._schema)

    const relDescriptor = find(belongsToRelations, ['relationModelName', modelName])

    if (!relDescriptor) throw new Error(`there is no belongsTo relation for "${modelName}"`)
