...
class SqlBuilder {
  ...
  _generateForeignKeysLines (fieldsOnly) {
    ...
    if (fieldsOnly) {
      if (!Array.isArray(fieldsOnly)) fieldsOnly = [fieldsOnly]
      relations = relations.filter((rel) => fieldsOnly.indexOf(rel.modelFieldName) > -1)
    }
  ...
  generateSelectFieldsPart (fieldsOnly) {
    ...
    let result = [this.idFieldClause]

    if (!fieldsOnly) {
      result = result.concat(this.columnsNames, this._generateForeignKeysLines())
    } else {
      if (!Array.isArray(fieldsOnly)) fieldsOnly = [fieldsOnly]

      result = result.concat(
        this.columnsNames.filter((col) => fieldsOnly.indexOf(col) > -1),
