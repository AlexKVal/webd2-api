...
const {find, omit, pickBy, transform} = require('lodash')
...
class SqlBuilder {
  constructor (schemaObject) {
    ...
    this.belongsToDescriptors = getBelongsToRelations(this._schema)
  ...
  getModelFieldName (modelName) {
    const belongsToDescriptor = find(this.belongsToDescriptors, ['relationModelName', modelName])

    if (!belongsToDescriptor) throw new Error(`there is no belongsTo relation for "${modelName}"`)

    return belongsToDescriptor.modelFieldName
  }
  ...
  _fieldsNamesForInsert (data) {
    ...
    .filter((rel) => data[rel.modelFieldName] != null) // only fields present in data
    .map((rel) => rel.fkField)
