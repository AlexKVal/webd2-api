...
class BaseModel {
  constructor ({db, name, schema, registry}) {
    ...
    if (!registry) throw new Error('registry is not provided')
    ...
    this._belongsToRelations = getBelongsToRelations(schema)

    this.attributesSerialize = Object.keys(omit(schema, ['id', 'tableName']))
    this.serializer = new Serializer({
      modelName: this.name,
      attributes: this.attributesSerialize,
      attributesOfRelations: attributesOfRelations(registry, this._belongsToRelations)
    })

    const modelNamesOfRelations = this._belongsToRelations.map((rel) => rel.relationModelName)
