...
const {castTypes, castTypesRow} = require('../utils/cast-types')
...
class BaseModel {
  constructor (db, name, schema) {
     ...
     * Serializer
     */
    const serializerOptions = { attributes: this.schema.attributes.slice() } // copy

    this.schema.belongsToRelations.forEach((rel) => {
      serializerOptions.attributes.push(rel.name)
      serializerOptions[rel.name] = { ref: 'id', attributes: rel.relationModel.schema.dataFieldsNames }
    })

    this.serializer = new Serializer(this.name, serializerOptions)
  ...
  fetchAll () {
    ...
    .then((parentRows) => this._fetchBelongsToRelations(parentRows))
    .then((dataSet) => this.serializer.serialize(dataSet))
  }

  _fetchBelongsToRelations (parentRows) {
    return Promise.all(parentRows.map((parentRow) => {
      return Promise.all(this.schema.belongsToRelations.map((rel) => {
        return rel.relationModel.get(parentRow[rel.fkName])
        .then((row) => castTypesRow(row, rel.relationModel.schema))
        .then((castRow) => {
          parentRow[rel.name] = castRow
