...
class BaseModel {
  ...
  /**
   * by default it validates OK
   * it is overridden for the real validation
   */
  validateBeforeUpdate (id, data) { return true }
  ...
  update (id, data) {
    ...
    .then(() => this.validateBeforeUpdate(id, data))
    .then(() => this.db.exec(this.sqlBuilder.update(id, data)))
  ...
  create (data, schemaMixin) {
    ...
    return this.db.exec(this.sqlBuilder.create(data, schemaMixin))
    .then(() => {
