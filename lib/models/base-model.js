...
class BaseModel {
  ...
  all () {
    debug(`${this.name}:all`)
    return this.db.exec(this.sqlBuilder.selectMany())
  ...
  get (id) {
    ...
    return this.db.exec(this.sqlBuilder.sqlOne(id))
    .then((rows) => {
  ...
  update (id, data) {
    ...
    .then((rows) => {
      ...
      return this.db.exec(this.sqlBuilder.sqlUpdate(id, data))
  ...
  create (data) {
    ...
    return this.db.exec(this.sqlBuilder.sqlCreate(data))
    .then(() => {
      return this.db.exec(this.sqlBuilder.sqlOneByData(data))
      .then((rows) => {
        if (rows.length === 0) {
