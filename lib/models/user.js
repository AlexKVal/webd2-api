...
class User extends BaseModel {
  sqlAll () {
    const foreignKeysFields = this.schema.getBelongsToRelations().map((rel) => {
      return `${rel.fkField} as ${rel.fkName}`
    })

    return `SELECT PersID as id,
    ${foreignKeysFields.concat(this.sqlBuilder.columnsNames).join(', ')}
    FROM sPersonal
    WHERE Hide=False
    ORDER BY name`
  }
  ...
  passwordVerifySql (id, password) {
    return `SELECT PersID as id,
      ${this.sqlBuilder.columnsNames.join(', ')}
      FROM sPersonal
      WHERE Hide=False
      AND PersID=${id}
