...
function sqlUpdate (id, data) {
  let setLines = generateUpdateSetPart(data, Model.schema)
...
const Model = {
  ...
  update (id, data) {
    ...
    debugSql(`user-account:update(${id})`, sqlUpdating)
...
const serializer = new Serializer('user-account', { attributes: Model.schema.dataFieldsNames.concat(userGroupRel.fkAs) })

Model.serialize = function serialize (records) {
  if (records == null) throw new Error('Model.serialize() records cannot be undefined')

  return serializer.serialize(castTypes(records, schema))
}
