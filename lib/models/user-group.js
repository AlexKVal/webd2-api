  ...
  fetchHasMany (relationModel) {
    ...
    return Promise.all([ this.all(), relationModel.all() ])
      .then(([modelRows, relationRows]) => {
        ...
        const options = { attributes: this.schema.columnsNames.concat(relationName) }
        options[relationName] = { ref: 'id', attributes: relationModel.schema.columnsNames }
