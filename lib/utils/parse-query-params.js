...
const { every, castArray, isObjectLike, isString, isUndefined, values, trim } = require('lodash')
...
function parseQueryParams (query) {
  ...
  if (!isUndefined(filter)) {
    if (isObjectLike(filter) && Object.keys(filter).length > 0) {
      ...
      const isValuesOK = every(values(filter), (val) => !isObjectLike(val))

      if (isFieldNamesOk && isValuesOK) parsedOptions.where = filter
    }
  }
