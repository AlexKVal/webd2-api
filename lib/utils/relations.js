...
function fetchBelongsToRelation ({model, belongsTo, query}) {
  ...
  return Promise.all([ model.all(query), belongsTo.all() ])
    .then(([modelRows, relationRows]) => {
      const castModelRows = castTypesRows(modelRows, model.schema.dataFields)
      const castRelationRows = castTypesRows(relationRows, belongsTo.schema.dataFields)
...
function fetchHasManyRelation ({model, hasMany}) {
  ...
  return Promise.all([ model.all(), hasMany.all() ])
    .then(([modelRows, relationRows]) => {
      const castModelRows = castTypesRows(modelRows, model.schema.dataFields)
      const castRelationRows = castTypesRows(relationRows, hasMany.schema.dataFields)
